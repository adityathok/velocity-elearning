<?php
if ( isset( $_POST['vdel_kelas_field'] ) && wp_verify_nonce( $_POST['vdel_kelas_field'], 'vdel_kelas_action' ) ) {
    update_option( 'vdel_kelas', $_POST['kelas'] );
...
$vdel_kelas = get_option( 'vdel_kelas' );
$vdel_kelas = $vdel_kelas?$vdel_kelas:['Kelas 1','Kelas 2','Kelas 3'];
?>
...
<form action="" method="POST" class="table-responsive">
    <table class="table table-hover table_vdel">
        ...
        <tbody>
            <?php foreach( $vdel_kelas as $i => $kl): ?>
                <tr> 
                    <td>
                        <input type="text" value="<?php echo $kl ; ?>" name="kelas[]" class="form-control form-control-sm border-0 bg-transparent">
                    </td>
                    <td class="text-end">
    ...
    <?php wp_nonce_field( 'vdel_kelas_action', 'vdel_kelas_field' ); ?>
